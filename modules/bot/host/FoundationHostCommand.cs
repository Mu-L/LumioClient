...
public static class FoundationHostCommand
{
    ...
    public static readonly byte[] WorldChange = WireCodec.EncodePack(new WorldChangeMessage(1UL, 0UL,
        new[]
        {
            ...
            new CreateRecord("player", FixtureSelf, Array.Empty<FieldValue>()),
    ...
    public static async Task<int> RunAsync(string[] args, CancellationToken cancellationToken)
    {
        ...
        if (parsed.Production) return BotHostOwnerPump.Run(() => RunProductionAsync(parsed, cancellationToken));
        if (!parsed.Foundation) return 2;
        if (!string.Equals(parsed.Transport, "local-embedded", StringComparison.OrdinalIgnoreCase)) return 3;
        if (!string.Equals(parsed.Fixture, "foundation-happy-path", StringComparison.OrdinalIgnoreCase)) return 4;
#if LUMIO_ENGINE_SDK
        ...
        var deps = new ClientSessionDependencies(connections, new ClientHandshakeFactory(), new HostCapability(),
            new HelloClassifier(), ingress, new InputCommandSource(ingress, new HostInputMapper()),
            IClientPersistenceFactory.CreateMemory().CreateVerifiedSessionArtifactSource(), writer,
            new WorldChangeRuntimePort(), new ClientReplicaFactory(), new ClientPredictionFactory(),
            new ImmediateGameplayScopeActivator(), new NullPresentationSink(), new FixtureMessageMap(),
            new NullClientOutboundMessageObserver());
        new ClientSessionFactory().Create(in deps, out IClientSession session);
        var host = new HeadlessBotHost(session, new DeterministicBotDriver(), ingress, new FoundationPeer(connections));
        int code = await Task.FromResult(BotHostOwnerPump.Run(() => host.RunAsync(new BotRunRequest(5, 0), cancellationToken)));
        ClientSessionSnapshot snap = session.GetSnapshot();
        if (snap.State == ClientSessionState.Faulted) return 1;
        if (snap.State != ClientSessionState.Closed) return 5;
        if (connections.Loopback.EncodeCalls < 1 || connections.Loopback.DecodeCalls < 1) return 6;
        return code;
    }

    private static Task<int> RunProductionAsync(HostArgs parsed, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(parsed.EngineNativePath))
        {
            ...
            return Task.FromResult(BlockedExitCode);
        }
        ...
        return Task.FromResult(BlockedExitCode);
#else
        return RunNativeProductionAsync(parsed, cancellationToken);
#endif
    }

#if LUMIO_NATIVE_LOADER
    private static async Task<int> RunNativeProductionAsync(HostArgs parsed, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await BotHostResidentLoop.RunAsync(residents, timer, logPath, releaseFlag, static ct => Task.Delay(1, ct), cancellationToken);
            return 0;
        }
        finally
        {
            for (int i = 0; i < bots.Count; i++) bots[i].Session.RequestClose(new SessionCloseRequest(false));
    ...
    private static ProductionBot CreateProductionBot(string server, string account, string logPath)
    {
        ...
        var options = new ClientEventPipelineOptions(8, 4, TimeSpan.FromSeconds(1));
        new ClientEventPipelineFactory().Create(in options, new InMemoryClientEventSink(8), out var writer);
        var transportOptions = new WebSocketTransportOptions(WebSocketTransportOptions.DefaultMaxMessageBytes,
            WebSocketTransportOptions.DefaultReceiveBufferBytes, TimeSpan.FromMinutes(5));
        byte[] initialFrame = JsonSerializer.SerializeToUtf8Bytes(new { connectionId = "c-" + account.ToLowerInvariant() });
        var endpoint = new ClientEndpoint(server, ReadOnlyMemory<byte>.Empty, ReadOnlyMemory<byte>.Empty,
            TimeSpan.FromSeconds(10), initialFrame, requiresMvpChannelAuth: false);
        var evidence = new ProductionChatInputEvidence(logPath, account);
        var deps = new ClientSessionDependencies(new WebSocketClientConnectionFactory(transportOptions),
            new ClientHandshakeFactory(), new HostCapability(), new HelloClassifier(), ingress,
            new InputCommandSource(ingress, new HostInputMapper()),
            IClientPersistenceFactory.CreateMemory().CreateVerifiedSessionArtifactSource(), writer,
            new WorldChangeRuntimePort(), new ClientReplicaFactory(), new ClientPredictionFactory(),
            new ImmediateGameplayScopeActivator(), new NullPresentationSink(), new JsonSessionMessageKindMap(), evidence);
        new ClientSessionFactory().Create(in deps, out IClientSession session);
    ...
    private static IEnumerable<string> EnumerateAccounts(string from, string to)
    {
        ...
        {
            string number = i < 100 ? i.ToString("D2", System.Globalization.CultureInfo.InvariantCulture)
    ...
    private static void SplitAccount(string account, out string prefix, out int number)
    {
        ...
        while (i >= 0 && char.IsDigit(account[i])) i--;
        prefix = i >= 0 ? account[..(i + 1)] : string.Empty;
        if (!int.TryParse(account.AsSpan(i + 1), System.Globalization.NumberStyles.None,
            System.Globalization.CultureInfo.InvariantCulture, out number)) number = 1;
    }
    ...
    private readonly struct ProductionBot
    {
        ...
        { AccountId = accountId; Session = session; Evidence = evidence; }
    ...
    internal readonly struct HostArgs
    {
        ...
        public static HostArgs Parse(string[]? args)
        {
            ...
            if (args != null)
            {
                ...
                {
                    if (string.Equals(args[i], "foundation", StringComparison.OrdinalIgnoreCase)) foundation = true;
                    else if (Match(args, i, "--transport", out string transportValue)) { i++; transport = transportValue; }
                    else if (Match(args, i, "--fixture", out string fixtureValue)) { i++; fixture = fixtureValue; }
                    else if (Match(args, i, "--server", out string serverValue)) { i++; server = serverValue; }
                    else if (Match(args, i, "--account-from", out string fromValue)) { i++; accountFrom = fromValue; }
                    else if (Match(args, i, "--account-to", out string toValue)) { i++; accountTo = toValue; }
                    else if (Match(args, i, "--engine-native", out string nativeValue)) { i++; engineNative = nativeValue; }
                    else if (Match(args, i, "--log-dir", out string logValue)) { i++; logDir = logValue; }
                }
            }
            if (args == null || args.Length == 0) foundation = true;
            return new HostArgs
            {
                Foundation = foundation, Production = !string.IsNullOrEmpty(server) && !foundation,
                Transport = transport, Fixture = fixture, Server = server, AccountFrom = accountFrom,
                AccountTo = accountTo, EngineNativePath = engineNative, LogDir = logDir
            };
        }
        private static bool Match(string[] args, int i, string name, out string value)
        {
            value = string.Empty;
            if (!string.Equals(args[i], name, StringComparison.OrdinalIgnoreCase) || i + 1 >= args.Length) return false;
            value = args[i + 1];
            return true;
        }
    ...
    private sealed class FoundationPeer : IBotTickHook
    {
        ...
        public FoundationPeer(CapturingConnectionFactory connections) { _connections = connections; }
        public void BeforeTick(int tick)
        {
            if (_connections.Loopback == null) return;
            byte[]? frame = tick switch { 0 => Hello, 1 => Welcome, 2 => WorldChange, 3 => Gap, 4 => WorldChange, _ => null };
            if (frame != null) _connections.Loopback.TryDeliverToClient(new EncodedFrame(frame));
        }
    }
    ...
    private sealed class FixtureMessageMap : ISessionMessageKindMap
    {
        ...
        {
            if (frame.Span.SequenceEqual(Welcome)) return SessionMessageKind.Welcome;
            if (frame.Span.SequenceEqual(WorldChange)) return SessionMessageKind.WorldChange;
            if (frame.Span.SequenceEqual(Gap)) return SessionMessageKind.Gap;
    ...
    private sealed class HostCapability : IPlatformCapabilityProvider
    {
        public ValueTask<PlatformCapabilityResult> QueryAsync(in PlatformCapabilityQuery query, CancellationToken cancellationToken)
        {
            ...
            return new(new PlatformCapabilityResult(query.Attempt, query.Generation, true));
        }
    }
